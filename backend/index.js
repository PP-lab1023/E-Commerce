...
mongoose.connect("mongodb+srv://celestepan1023:gs211023@cluster0.sstpk.mongodb.net/e-commerce")
...
app.post('/signup', async(req, res)=>{
  ...
  if (check) {
    return res.status(400).json({success: false, errors: "User with the same email addrss exists."})
  }
  let cart = {};
  for (let i = 0; i < 300; i++) {
    cart[i] = 0;
...
// Create API for deleting method
...
// Create API for getting all the products
...
const fetchUser = async (req, res, next)=>{
  ...
  if(!token){
    res.status(401).send({error:"Please authenticate user valid token"})
  }else{
    try{
...
// Create endpoint for removing products from cart data
